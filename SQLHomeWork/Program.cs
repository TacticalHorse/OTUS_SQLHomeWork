...
namespace SQLHomeWork
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            DropDB("sber_MBezruchenko");
            InitializeTemplateBase("sber_MBezruchenko");
            Console.ReadLine();

            //Пример скаляра
            Console.WriteLine($"Строк в BankBranch {SQLHelper.ExecuteScalar("SELECT COUNT(*) FROM \"BankBranch\";")}");
            Console.WriteLine(SQLHelper.ExecuteScalar("SELECT version();"));

            //Пример с параметрами
            SQLHelper.ExecuteNonQuery("INSERT INTO \"BankBranch\" (\"BranchName\") VALUES (@name)",
            ...
            foreach (var item in SQLHelper.ExecuteSQL("SELECT \"Id\", \"BranchName\" FROM  \"BankBranch\""))
            {
        ...
        private static void InitializeTemplateBase(string BaseName)
        {
            List<BankBranch> bankBranches = new List<BankBranch>();
            ...
            {
                Console.WriteLine("Таблица BankBranch создана." + Environment.NewLine);
            }
            SQLHelper.ExecuteSQL("DELETE FROM \"BankBranch\";");
            ...
            SQLHelper.ExecuteSQL("DELETE FROM \"Persons\";");
            if (FillPersonsTable(bankBranches) != null)
            {
                foreach (var item in SQLHelper.ExecuteSQL("SELECT \"Id\", \"BankBranchId\", \"Name\", \"Shurname\", \"Balance\" FROM  \"Persons\""))
                {
            ...
            SQLHelper.ExecuteSQL("DELETE FROM \"Transactions\";");
            ...
            foreach (var item in SQLHelper.ExecuteSQL("SELECT \"Id\", \"BankBranchId\", \"Name\", \"Shurname\", \"Balance\" FROM  \"Persons\"  ORDER BY \"Id\""))
            {
        ...
        #region DrawTable
        private static void DrawTable(List<BankBranch> bankBranches)
        {
        ...
        private static void DrawTable(List<Person> persons)
        {
            ...
            {
                Console.WriteLine("|{0,5}|{1,-30}|{2,-20}|{3,-20}|{4,10:N2}|", item.Id, item.BankBranch.BranchName, item.Name, item.Shurname, item.Balance);
            }
            Console.WriteLine("|".PadRight(90, '_') + "|");
        }
        private static void DrawTable(List<Transaction> transactions)
        {
            ...
            {
                Console.WriteLine("|{0,5}|{1,-30}|{2,-30}|{3,10}|{4,20}|", item.Id, $"{item.From.Shurname} {item.From.Name}", $"{item.To.Shurname} {item.To.Name}", item.Value, item.TimeStamp.ToString("yyyy-MM-dd HH:mm:ss"));
            }
            ...
            return SQLHelper.ExecuteNonQuery(
                @"CREATE TABLE ""Persons""(
                    ...
                    CONSTRAINT ""FK_Person_BBId_BankBranch_Id"" FOREIGN KEY(""BankBranchId"")
                        REFERENCES ""BankBranch"" (""Id"") ON DELETE CASCADE
                );");
        }
        ...
        {
            return SQLHelper.ExecuteNonQuery(
                @"INSERT INTO ""BankBranch"" (""BranchName"")
                    SELECT * FROM(VALUES
        ...
        {
            return SQLHelper.ExecuteNonQuery(
                @"INSERT INTO ""Persons"" (""BankBranchId"", ""Name"", ""Shurname"", ""Balance"")
                SELECT * FROM(VALUES " +
